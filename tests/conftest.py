...
def _inject(vm):
    ...
    fd,path=tempfile.mkstemp(prefix="asng-gltest-");_FILES.append(Path(path));os.write(fd,calldata.encode(message));os.lseek(fd,0,os.SEEK_SET);vm._original_stdin_fd=os.dup(0);os.dup2(fd,0);os.close(fd)
